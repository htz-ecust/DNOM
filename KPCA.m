...
%                                                   
MAX_MATRIX_SIZE = 10600; % You can change this number according your machine computational power
...
if nSmp > MAX_MATRIX_SIZE && ReducedDim < nSmp*EIGVECTOR_RATIO
    ...
    option = struct('issym',1,'disp',0);
    [eigvector, eigvalue] = eigs(K,ReducedDim,'lr',option);
    eigvalue = diag(eigvalue);
else
    [eigvector, eigvalue] = eig(K);
    eigvalue = diag(eigvalue);
    
    [dump, index] = sort(-real(eigvalue));
